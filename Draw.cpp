...
void DrawingClass::DispatchInput()
{
	MSG msg;
...
HRESULT DrawingClass::InitWindow(const char* ClassName)
{
	// Register class
	WNDCLASSEX wcex;
	...
	//? Create window
	g_hWnd = CreateWindowExA(WS_EX_TOPMOST |  WS_EX_LAYERED, ClassName, "", WS_POPUP, 0, 0, Width,Height, 0, 0, g_hInst, NULL);
	if (!g_hWnd)
		return E_FAIL;
...
HRESULT DrawingClass::InitDevice(bool Multisampling)
{
	HRESULT hr = S_OK;
...
HRESULT DrawingClass::DrawLine(XMFLOAT2 pos1,XMFLOAT2 pos2)
{
	HRESULT hr = S_OK;
...
HRESULT DrawingClass::DrawCircle(XMFLOAT2 center, UINT radius, UINT numVertices)
{
	HRESULT hr = S_OK;
...
void DrawingClass::Render()
{
	DrawLine(XMFLOAT2(0.0f, 0.0f), XMFLOAT2(0.0f,0.972f));
	DrawCircle(XMFLOAT2(Width/2, Height/2), 700, 299);
